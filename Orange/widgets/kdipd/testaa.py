...
from Orange.widgets.utils.experience_editor import ExperienceEditor
...
class OWFile(widget.OWWidget, RecentPathsWComboMixin):
    ...
    def __init__(self):
        ...
        self.testg = ExperienceEditor(self, ins, out)
        self.testg.init_data(6)
        # self.domain_editor = DomainEditor(self)
        # self.editor_model = self.domain_editor.model()
        box.layout().addWidget(self.testg)

        box = gui.hBox(self.controlArea)
        gui.button(
            box, self, "打开数据集",
            callback=lambda: self.get_grid_data(), autoDefault=False)
        gui.rubber(box)

    def get_grid_data(self):
        datas = self.testg.variables
        s = datas
