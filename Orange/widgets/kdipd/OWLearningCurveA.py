...
# [end-snippet-2]
        ...
        self.results = OrderedDict()#Return an instance of a dict subclass, supporting the usual dict methods. An OrderedDict is a dict that remembers the order that keys were first inserted. If a new entry overwrites an existing entry, the original insertion position is left unchanged. Deleting an entry and reinserting it will move it to the end.
    ...
    def set_dataset(self, data):
        ...
        self.commitBtn.setEnabled(self.data is not None)#true
    ...
    def _update_table(self):
        self.table.setRowCount(0)#设置表格行数
        self.table.setRowCount(len(self.curvePoints))#设置列数
        self.table.setColumnCount(len(self.learners))

        self.table.setHorizontalHeaderLabels(
            [learner.name for _, learner in self.learners.items()])#设置表头
        self.table.setVerticalHeaderLabels(
            ["{:.2f}".format(p) for p in self.curvePoints])#设置垂直标签
        ...
        for i in range(len(self.learners)):
            ...
            cwidth = self.table.columnWidth(i) #获取表格宽度
            self.table.setColumnWidth(i, max(sh, cwidth)) #设置某一行的宽度
