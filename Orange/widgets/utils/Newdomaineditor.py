...
class VarTableModel(QAbstractTableModel):
    ...
    def setData(self, index, value, role):
        ...
        # print('setData',row_data)
        if role == Qt.EditRole:
            if col == Column.name and not (value.isspace() or value == ""):
                row_data[col] = value
            elif col == Column.values and not (value.isspace() or value == ""):
                row_data[col] = value
                # print('value',value)
            elif col == Column.tpe:
...
class VarTypeDelegate(ComboDelegate):
    ...
    def setEditorData(self, combo, index):
        ...
        ##下面的判断是为了设置table的选项是多个来时只有"文本", "时间"
        if no_numeric:
            # Do not allow selection of numeric and datetime
            items = [i for i in self.items if i not in ("文本", "时间")]
...
class DomainEditor(QTableView):
    ...
    def __init__(self, widget):
        ...
        widget.contextAboutToBeOpened.connect(lambda args: self.set_domain(args[0])) ##生成self.variables
        # print('variable1',self.variables)
        widget.contextOpened.connect(lambda: self.model().set_variables(self.variables)) ##表格中输入数据
        widget.contextClosed.connect(lambda: self.model().set_variables([]))
        self.setModel(VarTableModel(self.variables, self)) ##添加模型到QTableView实例中
        self.setSelectionMode(QTableView.NoSelection) ##设置不可选择
        self.horizontalHeader().setStretchLastSection(True) ##最后一个section可延展
        self.setShowGrid(True)
        self.setEditTriggers(
            QTableView.SelectedClicked| QTableView.DoubleClicked ) ##这里设置触发事件
        self.setSizePolicy(
