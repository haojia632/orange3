...
class VarTableModel(QAbstractTableModel):  #Var表模型
    DISCRETE_VALUE_DISPLAY_LIMIT = 20  #离散值显示限制

    places = "特征（feature）", "目标（target）", "元（meta）", "跳跃（skip）"    #位置
    typenames = "明确（categorical)", "数字(numeric)", "文本(text)", "时间(datetime)"  #类型名称
    vartypes = DiscreteVariable, ContinuousVariable, StringVariable, TimeVariable    #变量类型
    ...
    def set_variables(self, variables): #重置
    ...
    def headerData(self, i, orientation, role=Qt.DisplayRole):
        if orientation == Qt.Horizontal and role == Qt.DisplayRole and i < 3:
            return ("输入", "上限", "下限")[i]
...
class DomainEditor(QTableView):
    ...
    @staticmethod
    def _iter_vals(x):
        """Iterate over values of sparse or dense arrays.迭代稀疏或密集数组的值。"""
        for i in range(x.shape[0]):
            yield x[i, 0]

    @staticmethod
    def _to_column(x, to_sparse, dtype=None):
        """Transform list of values to sparse/dense column array.将值列表转换为稀疏/密集列数组。"""
