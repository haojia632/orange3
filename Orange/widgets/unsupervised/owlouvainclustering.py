...
METRICS = [('欧氏', 'l2'), ('曼哈顿', 'l1')]


class OWLouvainClustering(widget.OWWidget):
    name = '鲁汶聚类'
    description = '检测最近邻居网络中的团体。'
    ...
    def __init__(self):
        ...
        pca_box = gui.vBox(self.controlArea, 'PCA 预处理')
        self.apply_pca_cbx = gui.checkBox(
            pca_box, self, 'apply_pca', label='应用PCA预处理',
            callback=self._invalidate_graph,
        )  # type: QCheckBox
        self.pca_components_slider = gui.hSlider(
            pca_box, self, 'pca_components', label='组分: ', minValue=2,
        ...
        self.metric_combo = gui.comboBox(
            graph_box, self, 'metric_idx', label='距离指标',
        ...
        self.apply_button = gui.auto_commit(
            self.controlArea, self, 'auto_commit', '应用', box=None,
